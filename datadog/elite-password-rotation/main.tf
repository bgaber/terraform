...
resource "datadog_synthetics_test" "elite_password_change" {
  ...
  config_variable {
    name = "NEW_${upper(each.key)}_ELITE_PASSWD"
    type = "global"
    id = datadog_synthetics_global_variable.new_password_variable.id
  }
