# Retrieve each parameter from SSM
data "aws_ssm_parameter" "keycloak_params" {
  for_each        = merge(flatten([
    for name, set in var.keycloak_parameter_sets : [
      { "${name}-customer" = set.customer },
      { "${name}-password" = set.password },
      { "${name}-realm"    = set.realm },
      { "${name}-url"      = set.url },
      { "${name}-userid"   = set.userid }
    ]
  ])...)
  name            = each.value
...
resource "datadog_synthetics_global_variable" "keycloak_admin_passwords" {
  for_each    = var.keycloak_parameter_sets
  name        = "${replace(replace(each.key, "/", "_"), "-", "_")}_KEYCLOAK_ADMIN_PASSWORD"
  description = "Keycloak admin password for ${each.key}"
  value       = data.aws_ssm_parameter.keycloak_params["${each.key}-password"].value
}
...
resource "datadog_synthetics_global_variable" "new_password_variable" {
  for_each    = var.keycloak_parameter_sets
  name        = "NEW_${upper(join("_", slice(split("/", each.key), 1, 3)))}_ELITE_PASSWD"
...
resource "datadog_synthetics_test" "elite_password_change" {
  for_each   = var.keycloak_parameter_sets
  name       = "${each.key} Password Rotation Synthetic Test"
  type       = "browser"
  status     = "paused"
  message    = "${each.key} Password Rotation Synthetic Test"
  ...
  request_definition {
    method = "GET"
    url    = data.aws_ssm_parameter.keycloak_params["${each.key}-url"].value
  }
  ...
  browser_step {
    ...
    type = "typeText"
    params {
      value = "{{ ${upper(join("_", slice(split("/", each.key), 1, 3)))}_KEYCLOAK_ADMIN_PASSWORD }}"
  ...
  # Step 8
  browser_step {
    name = "Click on link ${data.aws_ssm_parameter.keycloak_params["${each.key}-url"].value}"
    type = "click"
    params {
      element = "{\n          \"url\": \"${data.aws_ssm_parameter.keycloak_params["${each.key}-url"].value}/admin/master/console/#/realms/${data.aws_ssm_parameter.keycloak_params["${each.key}-realm"].value}/users\",\n          \"multiLocator\": {\n            \"ab\": \"/*[local-name()=\\\"html\\\"][1]/*[local-name()=\\\"body\\\"][1]/*[local-name()=\\\"div\\\"][1]/*[local-name()=\\\"div\\\"][1]/*[local-name()=\\\"div\\\"][1]/*[local-name()=\\\"div\\\"][1]/*[local-name()=\\\"table\\\"][1]/*[local-name()=\\\"tbody\\\"][1]/*[local-name()=\\\"tr\\\"][1]/*[local-name()=\\\"td\\\"][1]/*[local-name()=\\\"a\\\"][1]\",\n            \"at\": \"/descendant::*[@*[local-name()=\\\"ng-repeat\\\"]=\\\"user in users\\\"]/descendant::*[@href=\\\"#/realms/${data.aws_ssm_parameter.keycloak_params["${each.key}-realm"].value}/users/${data.aws_ssm_parameter.keycloak_params["${each.key}-userid"].value}\\\"]\",\n            \"cl\": \"/descendant::*[contains(concat(' ', normalize-space(@class), ' '), \\\" table \\\")]/descendant::*[contains(concat(' ', normalize-space(@class), ' '), \\\" clip \\\")][1]/*[local-name()=\\\"a\\\"][1]\",\n            \"co\": \"[{\\\"text\\\":\\\"${data.aws_ssm_parameter.keycloak_params["${each.key}-userid"].value}\\\",\\\"textType\\\":\\\"directText\\\"}]\",\n            \"ro\": \"//*[text()[normalize-space(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞŸŽŠŒ', 'abcdefghijklmnopqrstuvwxyzàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿžšœ')) = \\\"${data.aws_ssm_parameter.keycloak_params["${each.key}-userid"].value}\\\"]]\",\n            \"clt\": \"/descendant::*[contains(concat(' ', normalize-space(@class), ' '), \\\" table \\\")]/descendant::*[text()[normalize-space(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖØÙÚÛÜÝÞŸŽŠŒ', 'abcdefghijklmnopqrstuvwxyzàáâãäåæçèéêëìíîïðñòóôõöøùúûüýþÿžšœ')) = \\\"${data.aws_ssm_parameter.keycloak_params["${each.key}-userid"].value}\\\"]]\"\n          },\n          \"targetOuterHTML\": \"<a href=\\\"#/realms/${data.aws_ssm_parameter.keycloak_params["${each.key}-realm"].value}/users/${data.aws_ssm_parameter.keycloak_params["${each.key}-userid"].value}\\\" class=\\\"ng-binding\\\">${data.aws_ssm_parameter.keycloak_params["${each.key}-userid"].value}</a>\"\n        }"
    }
    allow_failure = false
    is_critical = true
    no_screenshot = false
