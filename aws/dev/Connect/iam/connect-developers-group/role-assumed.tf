...
resource "aws_iam_group" "connect_dev_gp" {
...
resource "aws_iam_group_policy_attachment" "cc-attach" {
  group      = aws_iam_group.connect_dev_gp.name
...
output "group_arn" {
  value = aws_iam_group.connect_dev_gp.arn
}

output "group_name" {
  value = aws_iam_group.connect_dev_gp.name
}
