...
resource "aws_kinesis_firehose_delivery_stream" "kinesis_firehose_stream" {
  ...
  name        = "exp-mgt-${replace(each.key, "/", "-")}-delivery-stream"
  ...
  # Ignore all changes - required because Decompress source records from Amazon CloudWatch Logs gets reset to Off for existing Firehose Streams
  # This means that Terraform can create and destroy the remote object but will never propose updates to it
...
data "aws_iam_policy_document" "inline_cloudwatch_policy_doc" {
  ...
  statement {
    ...
    resources = [
      ...
      "arn:aws:firehose:${var.region}:${local.account_id}:deliverystream/exp-mgt-${replace(k, "/", "-")}-delivery-stream"
    ]
  }
}
