resource "aws_api_gateway_account" "connect" {
...
resource "aws_api_gateway_method_settings" "v1" {
  depends_on = [
    aws_api_gateway_account.connect
  ]
  rest_api_id = aws_api_gateway_rest_api.rest_api.id
...
resource "aws_api_gateway_usage_plan" "connect" {
  name         = "process-twilio-message-UsagePlan"
  description  = "process-twilio-message Lambda Usage Plan"
