...
# commented out the DynamoDB table resource because DR will be using the Global DynamoDB table from Prod
#
...
resource "aws_lambda_layer_version" "connectproduserpoolstacklambdafunctionlayer69_efc6_a4" {
  ...
  layer_name          = "connect-dr-pool-node-fetch"
}

# Use the Prod Role
data "aws_iam_role" "connectproduserpoolstackconnectprodpretokengeneratorrole_e252_a9_fd" {
...
resource "aws_lambda_function" "pre_token_generator" {
  ...
  role          = data.aws_iam_role.connectproduserpoolstackconnectprodpretokengeneratorrole_e252_a9_fd.arn
...
resource "aws_cognito_user_pool_client" "connectproduserpoolstackuserpoolclient_f4_e238_d6" {
  ...
  callback_urls = [
    "https://dr.connect.compucom.com/api/auth/callback/cognito"
  ]
  name = "connect-dr-pool-client"
  explicit_auth_flows = [
    ...
    Statement = [
      {
        ...
        Effect = "Allow"
        Resource = [
          "arn:aws:logs:${var.region}:${local.account_id}:log-group:/aws/amplify/*:log-stream:*",
      ...
      },
      {
        ...
        Effect = "Allow"
        Resource = [
          "arn:aws:logs:${var.region}:${local.account_id}:log-group:/aws/amplify/*",
      ...
      },
      {
        Action = "dynamodb:*"
        Effect = "Allow"
        Resource = "arn:aws:dynamodb:${var.region}:${local.account_id}:table/connect-table"
      },
      {
        Action = "s3:*"
        Effect = "Allow"
