  ...
  policy = jsonencode({
    ...
    Statement: [
        ...
        },
        {
            ...
            Resource: [
              "arn:aws:logs:us-east-1:${local.account_id}:log-group:/aws/lambda/${var.athena_lambda_name}:*",
            ]
        },
        {
...
resource "aws_iam_role" "role_for_lambda" {
  name               = "role_for_lambda_${var.athena_lambda_name}"
...
resource "aws_lambda_function" "athena_lambda" {
  ...
  # filename = "${path.module}/artifacts/var.athena_lambda_archive"  # Ensure this is a zipped Python script
  ...
  environment {
    variables = {
      ...
      DD_API_KEY                = data.aws_ssm_parameter.datadog_api.value
      DD_APP_KEY                = data.aws_ssm_parameter.datadog_app.value
    ...
    Statement = [
      {
        ...
        Resource = [
            "arn:aws:lambda:us-east-1:${local.account_id}:function:${var.athena_lambda_name}:*",
...
resource "aws_scheduler_schedule" "poll_agent_version" {
  ...
  group_name  = "default"
  #start_date  = "2025-03-20T13:00:00Z"
