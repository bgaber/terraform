    ...
    mainSteps = [
      {
        ...
        inputs = {
          ...
          runCommand = [
            "set -euo pipefail",
            "",
            "# --- Idempotency check: skip if already installed and running ---",
            "if [ -x /opt/CrowdStrike/falconctl ] && systemctl is-active --quiet falcon-sensor; then",
            "  echo 'Falcon Sensor already installed. Skipping installation.'",
            "  exit 0",
            "fi",
            "",
    ...
    "mainSteps": [
      {
        ...
        "inputs": {
          ...
          "runCommand": [
            ...
            "",
            "# --- Pre-check: is Falcon Sensor already installed? ---",
            "$svc = Get-Service -Name 'CSAgent' -ErrorAction SilentlyContinue",
            "if ($svc -and $svc.Status -eq 'Running') {",
            "  Write-Host 'CrowdStrike Falcon Sensor already installed and running. Nothing to do.'",
            "  exit 0",
            "}",
            "",
