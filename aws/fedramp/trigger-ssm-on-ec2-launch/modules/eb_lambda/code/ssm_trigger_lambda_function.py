...
def is_kubernetes_worker(instance):
    """Detect if instance is a Kubernetes worker node by checking common cluster tags"""
    tags = instance.get("Tags", [])
    for tag in tags:
        if tag['Key'].startswith("kubernetes.io/cluster/"):
            logger.info(f"Instance {instance['InstanceId']} identified as a Kubernetes node")
            return True
    return False
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Skip installation of Falcon Sensor if Kubernetes Worker Node
        instance = ec2_client.describe_instances(InstanceIds=[instance_id])['Reservations'][0]['Instances'][0]

        if is_kubernetes_worker(instance):
            logger.info(f"Skipping instance {instance_id} as it is a Kubernetes worker node")
            return {
                'statusCode': 200,
                'body': f"Skipped Kubernetes worker node: {instance_id}"
            }
