...
#     """
#     Retrieve the value of the SecureString parameter 'DATADOG_API_KEY' from AWS SSM Parameter Store.
#     """
...
def get_instances_by_tag(account_id, tag_key):
...
def send_ssm_command_cross_account(account_ids, role_name, region, version):
    ...
    for account_id in account_ids:
        ...
        instances = get_instances_by_tag(account_id, "Patch Group")
        if not instances:
            logger.warning(f"No instances found with tag key of Patch Group in account {account_id}. Skipping SSM command.")
            continue
        ...
        logger.info(f"Parameters: version {version}, agentmajorversion {agentmajorversion}, agentminorversion {agentminorversion}")
