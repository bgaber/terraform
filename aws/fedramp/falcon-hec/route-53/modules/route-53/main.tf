data "aws_ssm_parameter" "falcon_hec_api_key" {
  name = "/crowdstrike/FALCON_HEC_API_KEY_ROUTE_53"
}

data "aws_ssm_parameter" "falcon_hec_api_url" {
  name = "/crowdstrike/FALCON_HEC_API_URL_ROUTE_53"
}

resource "aws_cloudwatch_event_connection" "connection" {
  name               = "${var.event_bridge_name}-connection"
  description        = "Falcon HEC connection for AWS Route 53"
...
resource "aws_cloudwatch_event_api_destination" "destination" {
  ...
  description                      = "Falcon HEC connector endpoint for AWS Route 53"
...
resource "aws_cloudwatch_event_rule" "event_rule" {
  ...
  description    = "HEC connector for AWS Route 53"
  event_pattern  = jsonencode({ "source": ["aws.route53"] })
}
