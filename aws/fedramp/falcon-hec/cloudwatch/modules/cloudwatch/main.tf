data "aws_ssm_parameter" "falcon_hec_api_key" {
  name = "/crowdstrike/FALCON_HEC_API_KEY_CLOUDWATCH"
}

data "aws_ssm_parameter" "falcon_hec_api_url" {
  name = "/crowdstrike/FALCON_HEC_API_URL_CLOUDWATCH"
}

resource "aws_cloudwatch_event_connection" "connection" {
  name               = "${var.event_bridge_name}-connection"
  description        = "Falcon HEC connection for AWS CloudWatch"
...
resource "aws_cloudwatch_event_api_destination" "destination" {
  ...
  description                      = "Falcon HEC connector endpoint for AWS CloudWatch"
  connection_arn                   = aws_cloudwatch_event_connection.connection.arn
  invocation_endpoint              = "${data.aws_ssm_parameter.falcon_hec_api_url.value}"
  http_method                      = "POST"
}

resource "aws_cloudwatch_event_rule" "event_rule" {
  name           = "${var.event_bridge_name}-rule"
  description    = "HEC connector for AWS CloudWatch"
  event_pattern  = jsonencode({ "source": ["aws.logs"] })
}
